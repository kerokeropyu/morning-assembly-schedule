...
def create_month_sheet(wb, year: int, month: int, members):
    ...
    # 既に同名のシートがある場合はいったん削除（常に最新状態で作り直す）
    if sheet_title in wb.sheetnames:
        old_ws = wb[sheet_title]
        wb.remove(old_ws)

    ws = wb.create_sheet(title=sheet_title)

    # 最初に自動でできる "Sheet" が残っているだけなら削除
    ...
    # ヘッダー（左上セル）
    ws.cell(row=1, column=1, value="氏名 / 日付")
    ...
    # 当番 ◯ を割り振り（1日から順番にメンバーをローテーション）
    for day in range(1, num_days + 1):
        duty_index = (day - 1) % num_members
        duty_row = duty_index + 2
        cell = ws.cell(row=duty_row, column=day + 1, value="◯")
